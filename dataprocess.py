...
with open("./grp_datas/wait_item_{}.csv".format(str(date.today()+timedelta(days=-1)).replace('-','')),'r',encoding='utf-8',errors='ignore') as f:
        fc = csv.reader(f,delimiter='$')
        cnt=1
        for row in fc:
                if cnt != 0:
                        try:
                                cur.execute(sql,(row[0],row[1],row[5],row[2] if row[2]!='' else 0,row[3] if row[3]!='' else 0,row[4] if row[4]!='' else 0,date.today(),))
                                connection.commit()
                                cnt += cur.rowcount
...
with open("./grp_datas/offer_info_{}.csv".format(str(date.today()+timedelta(days=-1)).replace('-','')),'r',encoding='utf-8',errors='ignore') as f:
        fc = csv.reader(f,delimiter='$')
        cnt=1
        for row in fc:
                if cnt != 0:
...
with open("./grp_datas/offer_item_{}.csv".format(str(date.today()+timedelta(days=-1)).replace('-','')),'r',encoding='utf-8',errors='ignore') as f:
        fc = csv.reader(f,delimiter='$')
        cnt=1
        for row in fc:
                if cnt != 0:
...
with open("./grp_datas/ent_ctg_{}.csv".format(str(date.today()+timedelta(days=-1)).replace('-','')),'r',encoding='utf-8',errors='ignore') as f:
        fc = csv.reader(f,delimiter='$')
        cnt=1
        for row in fc:
                if cnt != 0:
